...
log_file_path = os.path.join(log_dir, f"text_gradient_{time.time()}.log")
...
def main(period_len=25):
    ...
    training_pair_data, _ = create_pair_data(
        training_data, training_label, text_scaled, le
    )
    ...
    contrastive_train(text_model, sensor_model, loader, logger, num_epochs=num_epochs)

    torch.save(
        sensor_model.state_dict(),
        f"saved_models/text/sensor_period_{period_len}_model_weights.pth",
    )
    torch.save(
        text_model.state_dict(),
        f"saved_models/text/text_period_{period_len}_model_weights.pth",
    )
...
def main_evaluate(text_model, sensor_model, period_len=25):
    # set up logging
    logger = logging.getLogger()
...
def run_experiment(name, runs, **kwargs):
    ...
    for run_id in range(runs):
        ...
        main_evaluate(text_model, sensor_model, **kwargs)
