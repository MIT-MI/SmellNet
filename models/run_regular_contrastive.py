...
log_file_path = os.path.join(log_dir, f"contrastive_regular_{time.time()}.log")
...
def main(dropout=False, noisy=False):
    ...
    training_pair_data, _ = create_pair_data(
        training_X, training_label, gcms_scaled, le
    )
...
def main_evaluate(gcms_model, sensor_model):
    # set up logging
    logger = logging.getLogger()
...
def run_experiment(name, runs, **kwargs):
    ...
    for run_id in range(runs):
        ...
        main_evaluate(gcms_model, sensor_model)
